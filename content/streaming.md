...
How video actually reaches a player, one handoff at a time. It assumes you know
what a video file is and nothing else, and every section closes with what goes
wrong at that stage.

Written as background for [StreamPulse](https://github.com/sthavana/streampulse),
a synthetic health monitor for HLS and DASH. The callouts naming checks —
`period_gap`, `edge_stale` and the rest — refer to that project; they are kept
here because they say concretely what each failure looks like from outside.
Ad insertion has its own treatment in [the ad insertion guide](/guide).
...
inspection](https://github.com/sthavana/streampulse#looking-inside-the-media-optional) build.
...
[vantage](https://github.com/sthavana/streampulse#multi-vantage) label so the same channel probed from two
...
practice. What follows is the short version — the signalling, the two
architectures and the failure modes are covered properly in
[the ad insertion guide](/guide).
